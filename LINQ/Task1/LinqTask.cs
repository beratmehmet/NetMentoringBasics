...
namespace Task1
{
    public static class LinqTask
    {
        ...
        {
            return customers.Select(c => (c, suppliers.Where(s => s.Country == c.Country && s.City == c.City).GroupBy(s => s.Country + s.City).Select(g => g.First())));
        }
        ...
        {
            var q = customers.Where(c => c.Orders.Any());
            return q.Select(c => (c, c.Orders.Min(o => o.OrderDate)));
        }
        ...
        {
            return from customer in customers
                   where !int.TryParse(customer.PostalCode, out _) || string.IsNullOrEmpty(customer.Region) || !customer.Phone.Contains("(") || !customer.Phone.Contains(")")
                   select customer;
        }
        ...
        {
            var q = products.GroupBy(p => ProductRange(p, cheap, middle, expensive), resultSelector: (key, value) => new ValueTuple<decimal, IEnumerable<Product>> ( key, value));
            return q;
        }
        ...
        {
            var q = suppliers.Select(s => s.Country).Distinct().Distinct().OrderBy(a => a.Length).ThenBy(a => a);
